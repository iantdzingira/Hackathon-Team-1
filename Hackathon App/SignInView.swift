    ...
    func _body(configuration: TextField<Self._Label>) -> some View {
        HStack(spacing: 12) {
            Image(systemName: icon)
                .font(.callout)
                .foregroundColor(isFocused ? .customOrange : .secondary)
                .frame(width: 20)
        ...
        .background(
            RoundedRectangle(cornerRadius: 14)
                .stroke(isFocused ? .customOrange : Color(.systemGray5), lineWidth: isFocused ? 2 : 1)
                .background(Color(.systemBackground).cornerRadius(14))
                .shadow(color: isFocused ? .customOrange.opacity(0.1) : .clear, radius: 8, y: 2)
        )
    }
}
